...
export default function PytorchNodeBase({InputNum=1,OutputNum=1,children=<></>}:PytorchNodeBaseProps){
            ...
            <div className="handleContainer">{/* targetハンドラをまとめた部分 */}
                {
                    ...
                    Array.from({length:InputNum}).map((_,i)=>(
                        <Handle className="targetHandle" key={`target_${i}`} type="target" position={Position.Left}/>
                    ))
                }
            </div>
            ...
            <div className="handleContainer">{/* sourceハンドラをまとめた部分 */}
                {
                    ...
                    Array.from({length:OutputNum}).map((_,i)=>(
                        <Handle className="sourceHandle" key={`source_${i}`} type="source" position={Position.Right}/>
                    ))
                }
            </div>
        </div>
